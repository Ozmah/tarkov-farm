...
export const screenshots = sqliteTable(
	...
	(table) => [
		...
		uniqueIndex("screenshots_location_content_hash_unique").on(
			table.locationId,
			table.contentHash,
		),
		...
		check(
			"screenshots_preview_width_positive",
			sql`${table.previewWidth} IS NULL OR ${table.previewWidth} > 0`,
		),
		check(
			"screenshots_preview_height_positive",
			sql`${table.previewHeight} IS NULL OR ${table.previewHeight} > 0`,
		),
		check(
			"screenshots_preview_fields_complete",
			sql`(${table.previewPath} IS NULL AND ${table.previewWidth} IS NULL AND ${table.previewHeight} IS NULL) OR (${table.previewPath} IS NOT NULL AND ${table.previewWidth} IS NOT NULL AND ${table.previewHeight} IS NOT NULL)`,
		),
